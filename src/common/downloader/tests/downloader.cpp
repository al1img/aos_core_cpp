...
TEST_F(DownloaderTest, Download)
{
    ...
    auto err = mDownloader.Download(
        "http://localhost:8000/test_file.dat", mFilePath.c_str(), aos::UpdateItemTypeEnum::eService);
...
{
    auto err = mDownloader.Download("file://test_file.dat", mFilePath.c_str(), aos::UpdateItemTypeEnum::eService);
    EXPECT_EQ(err, aos::ErrorEnum::eNone);
...
TEST_F(DownloaderTest, DownloadLargeFileWithProgress)
{
    ...
    auto err = mDownloader.Download(
        "http://localhost:8001/large_test_file.dat", mFilePath.c_str(), aos::UpdateItemTypeEnum::eService);
